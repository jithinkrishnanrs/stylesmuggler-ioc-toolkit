...
and the `pub/media` web-shell sweep). The `hunting/` artifacts are used with external
tooling (Velociraptor, a Sigma-compatible SIEM/EDR) rather than these scripts.

## Provenance

Compiled from Sansec's advisory (published 2026-09-05, continuously updated; page
modified timestamp confirmed through at least 2026-09-11 14:14 UTC), Adobe's Security
...
with new indicators as recently as 2026-09-09, and whose `chronyd` process on the
Rust-implant side has been observed self-relaunching with no cron entry and a
PID-1 parent (see `file_paths.txt`). Treat every literal string here —
