#!/usr/bin/env bash
#
# stylesmuggler_scan.sh — read-only compromise scanner for the StyleSmuggler
...
if [[ -n "$MAGENTO_ROOT" && -d "$MAGENTO_ROOT" ]]; then
  ...
  if [[ $FINDINGS -eq 0 ]]; then
    ok "No injected PHP, trigger headers, or execution markers found in var/log/system.log or var/report."
  fi
else
  info "No --magento-root given (or path doesn't exist) — skipping log/report content scan. Re-run with --magento-root /path/to/magento"
fi
