...
| 2026-09-11 | Adobe's Experience League knowledge-base article for VULN-39341 reportedly updated to expand hotfix compatibility beyond the originally published version list — check Adobe's KB directly rather than relying on any static table's exact cutoff. |
| 2026-09-14, 13:27 UTC | **Sansec's advisory revised again with two major new sections.** A **third, distinct post-exploitation toolkit** confirmed: a remote-file-include backdoor edited directly into `vendor/magento/framework/App/View.php`, gated by a cookie (`gl_google_advisor_824808`) disguised as ad-tech tracking, fetching and executing a remote payload via `/tmp/tmp.log` and deleting it immediately after. Separately, an **"execution without the email" chain confirmed**: poisoning via PHP source in the query string of `POST /paypal/transparent/response/`, with observed payloads printing `MGPROOF::`/writing `mgproof717.txt` (proof-of-execution) and `MGKWSIM::` (direct command execution via a `kwc` request parameter) — reaching full RCE without ever rendering the failed-payment email. |
| 2026-09-16, 06:42 UTC | Sansec's advisory page modified timestamp confirmed again (table of contents unchanged from the Sept 14 revision; no specific new Sansec-authored content distinguished at this revision). Separately, corroborating community coverage reported: over 500 distinct source IPs observed by one exploit-tracking network since Sept 9; a public GitHub repository (created ~Sept 8) describing itself as a research lab reproduction of the full chain (not linked here); a plausible technical mechanism for the customer-custom-options delivery vector (a GIF+PHP polyglot upload exploiting a blacklist-based, not whitelist-based, extension filter); and CISA KEV metadata marking this entry as requiring forensic triage with ransomware-campaign use listed as "Unknown." |
